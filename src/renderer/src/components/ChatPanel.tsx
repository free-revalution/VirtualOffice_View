    ...
    <div className="flex-1 flex flex-col bg-white min-h-0 overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-slate-200 bg-slate-50 flex-shrink-0">
        <div className="flex items-center justify-between">
          <h2 className="text-slate-900 font-semibold">团队聊天</h2>
          <button
            ...
            >
              ...
              <div className={`flex-1 ${isCurrentUser ? 'flex flex-col items-end' : ''}`}>
                <div className="flex items-baseline gap-2 mb-1">
                  <span className={`text-xs text-slate-900 ${isCurrentUser ? 'order-2' : ''}`}>
                    {message.userName}
                  </span>
                ...
                <div className={`inline-block max-w-xs rounded-lg px-3 py-2 ${
                  ...
                  {message.translated && autoTranslate && (
                    <div className={`mt-1.5 pt-1.5 border-t ${
                      isCurrentUser ? 'border-blue-500' : 'border-slate-300'
                    }`}>
      ...
      <div className="p-3 border-t border-slate-200 flex-shrink-0">
        <div className="flex items-end gap-2">
          <div className="flex-1 bg-slate-50 rounded-lg border border-slate-200 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500">
            <textarea
              value={inputMessage}
              onChange={(e) => setInputMessage(e.target.value)}
              onKeyPress={handleKeyPress}
              placeholder="输入消息... (Enter发送，Shift+Enter换行)"
              className="w-full px-3 py-2 bg-transparent resize-none focus:outline-none text-sm text-black placeholder:text-slate-400"
              rows={2}
            />
          </div>
          ...
          <button
            ...
            className="flex-shrink-0 w-10 h-10 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white rounded-lg flex items-center justify-center transition-colors"
          >
