...
import { LogIn, UserPlus, Mail, Lock, User, Eye, EyeOff, Sparkles, Users, AlertCircle, CheckCircle } from 'lucide-react';
...
export function Auth({ onLogin }: AuthProps): React.JSX.Element {
    ...
    try {
      if (isLogin) {
        ...
        if (!formData.email || !formData.password) {
          setMessage({ text: '请填写邮箱和密码', type: 'error' });
          setIsSubmitting(false);
          return;
        }
        
        // 模拟登录成功
        console.log('登录请求:', { email: formData.email });
        ...
        if (formData.password.length < 6) {
          setMessage({ text: '密码长度至少为6位', type: 'error' });
          setIsSubmitting(false);
          return;
        }
        
        // 模拟注册成功
        console.log('注册请求:', { name: formData.name, email: formData.email });
    ...
    <div className="min-h-screen w-full relative overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20 animate-slide-up">
          ...
          {/* 消息提示 */}
          {message.text && (
            <div className={`mt-4 p-3 rounded-lg flex items-center gap-2 animate-fade-in ${message.type === 'success' ? 'bg-green-500/20 border border-green-400/30' : 'bg-red-500/20 border border-red-400/30'}`}>
